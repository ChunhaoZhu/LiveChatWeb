        ...
        new Promise((resolve, reject) => {
            ...
            db.collection('conversations').find({
                ...
                if (err){
                    ...
                    // var closestTime;
                    // var closestCon;
                    // conversations.forEach((dialog) => {
                    //     if (!closestTime && before > dialog.timestamp) closestTime = before - dialog.timestamp;
                    //     if (before > dialog.timestamp && before - dialog.timestamp < closestTime){
                    //         closestCon = dialog;
                    //         closestTime = before - dialog.timestamp;
                    //     }
                    // })
                    // resolve(closestCon);
                    var dialog = conversations.sort((a, b) => {
                        return b.timestamp - a.timestamp
                    }).filter(conv => {
                        return conv.timestamp < before;
                    })
                    resolve(dialog[0]);
                } else {
                    resolve(null);
...
//                 reject(new Error('Not enough information in conversation.'))
//             }
